## 👋 Hi there, I'm Sim

📱 Mobile App Developer | 👨‍🏫 Mobile Development Instructor  
🚀 Passionate about building modern, user-friendly mobile applications and mentoring future developers.

---

## 🛠 Skills & Technologies

### 📱 Mobile Development
<p>
  <img src="https://skillicons.dev/icons?i=flutter,dart,firebase" />
</p>

### ⚙️ Backend & APIs
<p>
  <img src="https://skillicons.dev/icons?i=php,laravel,nodejs,express,mysql,postgres,mongodb" />
</p>

### 🧠 Programming
<p>
  <img src="https://skillicons.dev/icons?i=cpp,cs,java" />
</p>

### 🧰 Tools & Platforms
<p>
  <img src="https://skillicons.dev/icons?i=git,github,vscode,figma,androidstudio" />
</p>

---

## 📌 What I Do
- Build Cross-Platform Mobile Applications using Flutter
- Develop REST APIs and integrate them into mobile apps
- Implement Authentication, Localization, and State Management
- Integrate Firebase services (Authentication, Firestore, FCM)
- Design responsive and user-friendly mobile interfaces
- Teach Flutter, Dart, Firebase, and Mobile App Development
- Guide students through real-world mobile projects

📫 Contact me:
- Facebook: [https://facebook.com/thiv.slot.5](https://web.facebook.com/e9mn0f0uyq/)
- GitHub: https://github.com/seangleangsim006-cmd
- Telegram: https://t.me/seangleangsim
